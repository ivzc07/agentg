...
export function AttendanceStrip({ cells, compact = false }: AttendanceStripProps) {
  ...
  const missedDates = cells
    .filter((cell) => cell.state === "miss")
    .map((cell) => t("sr_missed").replace("{date}", cell.on));

  return (
    <>
      <span
        className={`strip inline-flex items-center ${compact ? "gap-px" : "gap-0.5"}`}
        aria-hidden="true"
      >
        {cells.map((cell, i) => {
      ...
      {missedDates.length > 0 && <span className="sr">{missedDates.join(" ")}</span>}
    </>
  );
}
