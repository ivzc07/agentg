...
- **Referral mechanics — consent-gated coach ping.** ~~"Want me to flag this to your coach?" — on yes, message the gym's coach on Telegram and log to `member_notes`; on no, just log. The member controls what's shared.~~ **Superseded by [docs/spec-dashboard.md](spec-dashboard.md) §Safety flags ([#80](https://github.com/ivzc07/agentg/issues/80)):** the consent ask is dropped — the Agent always logs the flag as a `safety`-kind Note and pings every Coach of the Gym with an authenticated deep link to the Member's page.
