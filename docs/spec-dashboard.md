...
- **A named terminal policy**: eight attempts with exponential, jittered backoff (5s doubling to a 300s cap, +/-25% jitter so a provider outage does not retry a whole batch in lockstep), then the job is retired as `failed` with `failure_kind = "retry_exhausted"`. Retrying forever would turn one dead channel into unbounded background load and hide the failure; a retired job stays **queryable** by Gym and by failure kind instead.
