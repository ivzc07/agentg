...
- **Rendering**: server-rendered HTML from typed **Python f-string renderers** (no template engine) plus small vanilla JS snippets; new partial-page interactivity goes through **vendored htmx** returning HTML fragments from the same renderers. No frontend build step, no SPA, no API layer ([ADR 0003](adr/0003-dashboard-stays-server-rendered.md)).
