...
with **zero feedback to the Member** for the whole 3–7 s.
...
| 6 | Defer `reset_rhythm` past the reply, or make it one conditional UPDATE | `runtime.py:93` | S | −1–2 queries before the LLM |
| 7 | Memoize the active Routine on `MemberContext` (loaded 3× per turn) | `snapshot.py:32`, `tools.py:65`, `advice.py:52` | S | −4–8 queries |
| 8 | Move compaction off the critical path + add a convergence guard | `runtime.py:95`, `compaction.py:63-86` | M | −2–6 s on compaction turns; kills a latent per-turn doubling |
...
| 13 | Streaming replies (`Runner.run_streamed`) | `runtime.py:99` | M/L | −1–1.8 s perceived on every turn |
...
the whole turn (`runtime.py:81`), so one hung call wedges that Member's chat for 10 minutes.
Fix: `extra_args={"_skip_mcp_handler": True, "timeout": 30, "num_retries": 1}`.

**P2 — a transient compaction failure kills a good message.** `runtime.py:95` awaits
