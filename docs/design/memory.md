...
**Growth policy** (the SDK stores raw items and has no built-in summarizer, so this is ours): before each run, if the session exceeds ~50 items, compact — ask the model for a short summary of the oldest turns, write anything durable to member notes, then replace the old items with the summary item (`Session.get_items`/`clear_session`/`add_items` make this a plain operation). History is a working buffer, not an archive; nothing of record lives only in it (facts are in tables, durables in notes).
